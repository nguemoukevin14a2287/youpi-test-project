<script setup>
	...
	const save = () => {
		if(isUpdate.value){
			form.transform(data => data).put(route('employers.update'), {
				onSuccess: page => {
...
<template>
			...
			<form @submit.prevent="save">
					...
					<div class="self-end">
						...
						<div class="flex items-center gap-4">
							<label v-for="(s, j) in [{val: 'b', title: 'Homme'}, {val: 'g', title: 'Femme'}]" :for="`sexe-${s.val}-input`" :key="`sexe-${s.val}`" class="flex items-center gap-1 cursor-pointer">
								<input type="radio" name="sexe" v-model="form.sexe" class="checkbox" :value="s.val" :id="`sexe-${s.val}-input`">
								<span>{{ s.title }}</span>
							</label>
						</div>
					</div>
					...
					<div class="col-span-2 bg-sky-100 p-2">
						...
						<div class="md:grid gap-2 grid-cols-2">
							<div class="flex items-center gap-2 mt-2" v-for="(strength, i) in form.strengths" :key="`strength-${i}`">
								<input :id="`strength-input-${i}`" class="form-control grow" v-model="form.strengths[i]" type="text" required placeholder="Entrer une qualite" />
								<button type="button" v-if="form.strengths.length > 1" class="btn btn-danger" @click="form.strengths.splice(i, 1)">
						...
						<div class="text-right mt-4">
							<button type="button" class="btn btn-primary" @click="form.strengths.push('')" :disabled="form.strengths.filter(stren => stren.trim().length == 0).length > 0">
				...
				<div class="text-center mt-4"><button class="btn btn-success" type="submit">Sauvegarder</button></div>
			</form>
</template>